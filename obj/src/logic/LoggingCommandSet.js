...
class LoggingCommandSet extends pip_services_commons_node_1.CommandSet {
    ...
    makeWriteMessagesCommand() {
        return new pip_services_commons_node_2.Command("write_messages", new pip_services_commons_node_5.ObjectSchema(true)
            .withRequiredProperty('messages', new pip_services_commons_node_6.ArraySchema(new LogMessageV1Schema_1.LogMessageV1Schema())), (correlationId, args, callback) => {
            let messages = args.get("messages");
            _.each(messages, (m) => {
                m.time = pip_services_commons_node_9.DateTimeConverter.toNullableDateTime(m.time);
            });
